...
public class Main_MultiThreSquareExperiment {
...
//		List<Double> threList = Arrays.asList(1.5, 3., 5., 10.);
		List<Double> threList = Arrays.asList(5.);
...
//		double[] squareSizes = {1.};
		...
		if(args.length == 3){
			...
			String[] squareSizeStrs = args[4].split(",");
			squareSizes = new double[squareSizeStrs.length];
			for (int j = 0; j < squareSizes.length; j++) {
				double sqSize = Double.valueOf(squareSizeStrs[j]);
				squareSizes[j] = sqSize;
			}
			...
			// threList format "1.0,1.5,2.0"
			String[] squareSizeStrs = args[4].split(",");
			System.out.println("Square Sizes are: ");
			squareSizes = new double[squareSizeStrs.length];
			for (int j = 0; j < squareSizes.length; j++) {
				double sqSize = Double.valueOf(squareSizeStrs[j]);
				squareSizes[j] = sqSize;
			...
			// threList format "1.0,1.5,2.0"
			System.out.println("Thresholds are:");
			for (int i = 0; i < args.length-5; i++) {
				String[] threStrs = args[i+5].split(",");
